...
factorVarQuantVarCorrelation <- function(data_frame, factor_column_name, quant_column_name) {
  ...
  #TODO: Shapiro-Wilk Test by factor column (Robyn)
  # Shapiro-Wilk normality test for each level of the factor variable
  
  n <- nlevels(factor(data_frame[,factor_column_name]))
  
  for (i in 1:n){
    s <- shapiro.test((data_frame[,quant_column_name])[levels(data_frame[,factor_column_name]) == levels(data_frame[,factor_column_name])[i]])
  ...
  #TODO: Kruskal-Wallis test (Robyn)
  
  kruskal.test(data_frame[,quant_column_name] ~ data_frame[,factor_column_name])
